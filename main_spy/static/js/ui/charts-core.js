	...
	fetch_data: function() {
		...
		if (this.prev_data_url != this.data_url) {
			...
			for (var i = 0, l = filters_results.length; i < l; i++) {
				if (filters_results[i] != this.prev_filters_results[i] && filters[i].invalidating(filters_results[i], this.prev_filters_results[i])) {
					cleaning_needed = true
					break
				}
			}
		}
		if (this.cleaning_needed) {
			this.clean()
			this.ajax_token++
		}
		this.prev_filters_results = filters_results
		this.prev_data_url = prev_data_url
