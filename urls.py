...
urlpatterns = patterns('',
    ...
    url(r'group/add/(?P<gid>\d+)/$', 'main_spy.crud_views.add_group'),
    url(r'group/delete/(?P<gid>\d+)/$', 'main_spy.crud_views.delete_group'),
    ...
    url(r'series/group(?P<group_id>\d+)/(?P<stat_id>[a-z_0-9]+)/(?P<time_start>[0-9]+)/(?P<time_end>[0-9]+)/$', 'main_spy.stat_views.get_series_group_wide'),
    url(r'series/all_social_stats/group(?P<group_id>\d+)/(?P<time_start>[0-9]+)/(?P<time_end>[0-9]+)/$', 'main_spy.stat_views.get_series_group_wide_all_social_stats'),
    url(r'series/group(?P<group_id>\d+)/social_dynamics/(?P<content_types>[a-z_]*)/(?P<stat_id>[a-z_]+)/(?P<time_start>[0-9]+)/(?P<time_end>[0-9]+)/$', 'main_spy.stat_views.get_series_for_posts'),
    url(r'series/group(?P<group_id>\d+)/social_dynamics_all/(?P<content_types>[a-z_,]*)/(?P<time_start>[0-9]+)/(?P<time_end>[0-9]+)/$', 'main_spy.stat_views.get_all_stats_series_for_posts'),
    url(r'group(?P<group_id>\d+)/demogeo_snapshot/(?P<time>[0-9]+)/$', 'main_spy.stat_views.get_demogeo_snapshot')
)
